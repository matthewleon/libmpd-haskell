...
import           Control.Concurrent.Async.Lifted.Safe (Forall, Pure, Async, async)
...
idleAsync :: (MonadMPD m, MonadBaseControl IO m, Forall (Pure m))
          => [Subsystem] -> m (Async [Subsystem])
